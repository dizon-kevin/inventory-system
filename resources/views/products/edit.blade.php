...
<head>
    ...
    <title>Edit Product — Storix</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .snapshot-name { font-size: .96rem; font-weight: 700; color: var(--tp); }
        .snapshot-sku  { font-size: .75rem; font-family: 'Space Mono',monospace; color: var(--tm); margin-top: 2px; }

        .snapshot-meta { display: flex; align-items: center; gap: .6rem; margin-top: 5px; flex-wrap: wrap; }
        .snapshot-badge { display: inline-flex; align-items: center; gap: 4px; padding: 2px 8px; border-radius: 20px; font-size: .67rem; font-weight: 600; }
        .snapshot-badge.green  { background: rgba(34,197,94,.1);  color: #16a34a; }
        .snapshot-badge.yellow { background: rgba(251,191,36,.1); color: #b45309; }
        .snapshot-badge.red    { background: rgba(239,68,68,.1);  color: #dc2626; }
        .snapshot-price { font-family: 'Space Mono',monospace; font-size: .8rem; font-weight: 700; color: var(--tp); }
        .snapshot-id { font-size: .7rem; color: var(--tm); }

        /* ── FORM LAYOUT ── */
        .form-layout { display: grid; grid-template-columns: 1fr 320px; gap: 1.4rem; align-items: start; }

        /* FORM CARD */
        .form-card { background: var(--card); border: 1px solid var(--border); border-radius: 14px; overflow: hidden; animation: cardIn .5s .06s cubic-bezier(.22,1,.36,1) both; }

        .form-section { padding: 1.3rem 1.5rem; border-bottom: 1px solid var(--border); }
        .form-section:last-child { border-bottom: none; }

        .form-section-title { font-size: .78rem; font-weight: 700; letter-spacing: .07em; text-transform: uppercase; color: var(--tm); margin-bottom: 1.1rem; display: flex; align-items: center; gap: 8px; }
        .form-section-title::before { content: ''; width: 3px; height: 14px; background: var(--teal); border-radius: 2px; }
        ...
        .field-select,
        .field-textarea {
            width: 100%; padding: .75rem 1rem; background: var(--light);
            border: 1.5px solid var(--border); border-radius: 9px;
            color: var(--tp); font-family: 'Sora',sans-serif; font-size: .875rem;
        ...
        .field-input:focus, .field-select:focus, .field-textarea:focus {
            border-color: rgba(0,168,120,.5); background: #ffffff;
            box-shadow: 0 0 0 3.5px rgba(0,168,120,.09);
        }

        .field-input:hover:not(:focus),
        .field-select:hover:not(:focus),
        .field-textarea:hover:not(:focus) { border-color: rgba(12,26,20,.14); }
        ...
        .field-prefix { position: absolute; left: 0; top: 0; bottom: 0; width: 38px; display: flex; align-items: center; justify-content: center; color: var(--tm); font-size: .85rem; font-weight: 600; border-right: 1.5px solid var(--border); pointer-events: none; border-radius: 9px 0 0 9px; }
        ...
        /* Image card */
        .img-card { background: var(--card); border: 1px solid var(--border); border-radius: 14px; overflow: hidden; }
        ...
        .current-img-overlay {
            ...
            background: rgba(6,10,17,0.5); display: flex;
            flex-direction: column; align-items: center; justify-content: center;
            gap: 6px; color: #e2eeea; font-size: .78rem; font-weight: 500;
        ...
        .img-preview-wrap {
            ...
            border: 2px dashed rgba(0,168,120,.2); border-radius: 10px;
            display: flex; flex-direction: column; align-items: center; justify-content: center;
            background: var(--light); margin-bottom: 1rem; overflow: hidden;
            cursor: pointer; transition: border-color .2s, background .2s;
            position: relative;
        }

        .img-preview-wrap:hover { border-color: rgba(0,168,120,.4); background: rgba(0,168,120,.03); }
        .img-preview-wrap.has-img { border-style: solid; }

        #imagePreview { width: 100%; height: 100%; object-fit: cover; display: none; border-radius: 8px; }

        .img-placeholder { display: flex; flex-direction: column; align-items: center; gap: .55rem; color: var(--tm); text-align: center; padding: .85rem; }
        .img-placeholder .ph-icon { width: 38px; height: 38px; border-radius: 10px; background: var(--teal-dim); display: flex; align-items: center; justify-content: center; color: #00a878; }
        .img-placeholder p { font-size: .74rem; color: var(--tm); line-height: 1.5; }
        .img-placeholder span { font-size: .67rem; color: rgba(12,26,20,.25); }

        .img-change-overlay { display: none; position: absolute; inset: 0; background: rgba(6,10,17,.52); border-radius: 8px; align-items: center; justify-content: center; flex-direction: column; gap: 5px; color: #e2eeea; font-size: .75rem; font-weight: 500; }
        .img-preview-wrap.has-img:hover .img-change-overlay { display: flex; }

        #imageInput { display: none; }

        .btn-upload { width: 100%; padding: .6rem 1rem; border: 1.5px solid rgba(0,168,120,.22); border-radius: 9px; background: rgba(0,168,120,.05); color: #00a878; font-family: 'Sora',sans-serif; font-size: .78rem; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; transition: background .18s, border-color .18s; }
        .btn-upload:hover { background: rgba(0,168,120,.11); border-color: rgba(0,168,120,.38); }

        .img-hint { font-size: .68rem; color: var(--tm); text-align: center; margin-top: .55rem; line-height: 1.5; }
        .img-keep  { font-size: .72rem; color: var(--tm); display: flex; align-items: center; gap: 5px; margin-top: .55rem; }
        .img-keep svg { color: var(--teal); }

        /* Tabs: current vs new */
        .img-tabs { display: flex; gap: 4px; margin-bottom: .85rem; }
        .img-tab { flex: 1; padding: .48rem; border: 1.5px solid var(--border); border-radius: 7px; background: none; font-family: 'Sora',sans-serif; font-size: .74rem; font-weight: 500; color: var(--ts); cursor: pointer; transition: all .15s; text-align: center; }
        .img-tab.active { background: var(--dark); color: #d8f0e8; border-color: transparent; }

        /* Form actions */
        .form-actions { display: flex; align-items: center; justify-content: flex-end; gap: .75rem; padding: 1.1rem 1.5rem; border-top: 1px solid var(--border); background: #fafbfa; }
        .btn-cancel { display: inline-flex; align-items: center; gap: 6px; padding: .68rem 1.2rem; border: 1.5px solid var(--border); border-radius: 9px; background: none; color: var(--ts); font-family: 'Sora',sans-serif; font-size: .84rem; font-weight: 500; cursor: pointer; text-decoration: none; transition: background .15s, color .15s; }
        .btn-cancel:hover { background: var(--light); color: var(--tp); }
        .btn-save { display: inline-flex; align-items: center; gap: 7px; padding: .68rem 1.4rem; background: #0a1a15; border: none; border-radius: 9px; color: #d8f0e8; font-family: 'Sora',sans-serif; font-size: .84rem; font-weight: 600; cursor: pointer; letter-spacing: .03em; transition: background .2s, transform .15s, box-shadow .2s; }
        ...
        @media (max-width: 1100px) { .form-layout { grid-template-columns: 1fr 280px; } }
    ...
    <div class="main">
        ...
        <main class="content">
            ...
            <div class="page-header">
                ...
                <p class="page-subtitle">Update product details, pricing, stock, and image</p>
            </div>

            {{-- Product snapshot --}}
            <div class="product-snapshot">
                @if($product->image)
                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="snapshot-thumb" id="snapshotThumb" />
                @else
                    <div class="snapshot-thumb-placeholder" id="snapshotThumb">
                ...
                <div class="snapshot-info">
                    <p class="snapshot-name" id="snapshotName">{{ $product->name }}</p>
                    <p class="snapshot-sku">SKU: <span id="snapshotSku">{{ $product->sku }}</span></p>
                    <div class="snapshot-meta">
                        @if($product->stock_status === 'In Stock')
                            <span class="snapshot-badge green" id="snapshotStatus">In Stock</span>
                        @elseif($product->stock_status === 'Low Stock')
                            <span class="snapshot-badge yellow" id="snapshotStatus">Low Stock</span>
                        @else
                            <span class="snapshot-badge red" id="snapshotStatus">Out of Stock</span>
                        @endif
                        <span class="snapshot-price" id="snapshotPrice">${{ number_format($product->price, 2) }}</span>
                        <span class="snapshot-id">#{{ $product->id }}</span>
                    </div>
                </div>
            </div>
                                ...
                                <div class="form-grid-2">
                                    ...
                                    <div class="field">
                                        ...
                                        <input id="name" name="name" type="text" class="field-input"
                                               ...
                                               oninput="document.getElementById('snapshotName').textContent=this.value||'—'"
                                               required />
                                        @error('name')<span class="field-error"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>{{ $message }}</span>@enderror
                                    </div>

                                    <div class="field">
                                        <label class="field-label" for="sku">SKU <span class="req">*</span></label>
                                        <input id="sku" name="sku" type="text" class="field-input"
                                               value="{{ old('sku', $product->sku) }}"
                                               oninput="document.getElementById('snapshotSku').textContent=this.value"
                                               required />
                                        @error('sku')<span class="field-error"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>{{ $message }}</span>@enderror
                                    </div>
                                    ...
                                    <div class="field">
                                        ...
                                        @error('category_id')<span class="field-error"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>{{ $message }}</span>@enderror
                                    </div>

                                    <div class="field col-span-2">
                                        <label class="field-label" for="description">Description</label>
                                        <textarea id="description" name="description" class="field-textarea" rows="3"
                                                  placeholder="Describe the product features and specifications...">{{ old('description', $product->description) }}</textarea>
                                        @error('description')<span class="field-error"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>{{ $message }}</span>@enderror
                                    </div>
                                ...
                                <div class="form-grid-2">
                                    ...
                                    <div class="field">
                                        ...
                                        <div class="field-input-wrap">
                                            <input id="price" name="price" type="number" class="field-input"
                                                   value="{{ old('price', $product->price) }}"
                                                   min="0" step="0.01" placeholder="0.00"
                                                   oninput="var v=parseFloat(this.value)||0;document.getElementById('snapshotPrice').textContent='$'+v.toFixed(2)"
                                                   required />
                                            <span class="field-prefix">$</span>
                                        </div>
                                        @error('price')<span class="field-error"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>{{ $message }}</span>@enderror
                                    </div>

                                    <div class="field">
                                        <label class="field-label" for="quantity">Quantity <span class="req">*</span></label>
                                        <input id="quantity" name="quantity" type="number" class="field-input"
                                               value="{{ old('quantity', $product->quantity) }}"
                                               min="0" placeholder="0" required />
                                        @error('quantity')<span class="field-error"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>{{ $message }}</span>@enderror
                                    </div>
                    ...
                    <div class="right-col">
                        <div class="img-card">
                            ...
                            <div class="img-card-body">
                                ...
                                @if($product->image)
                                    <div class="img-tabs">
                                        <button type="button" class="img-tab active" id="tabCurrent" onclick="showTab('current')">Current</button>
                                        <button type="button" class="img-tab" id="tabNew" onclick="showTab('new')">Replace</button>
                                    </div>
                                @endif
                                ...
                                @if($product->image)
                                    <div id="panelCurrent">
                                        <div class="current-img-wrap" onclick="showTab('new')">
                                ...
                                <div id="panelNew" style="{{ $product->image ? 'display:none' : '' }}">
                                    ...
                                    <button type="button" class="btn-upload" onclick="document.getElementById('imageInput').click()">
                                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                                        Choose New Image
                                    </button>

                                    <p class="img-hint">Leave empty to keep the current image</p>

                                    @error('image')
                                        <span class="field-error" style="display:flex;margin-top:.5rem">
...
<script>
    ...
    function showTab(tab) {
        var current = document.getElementById('panelCurrent');
        var newPanel = document.getElementById('panelNew');
        var tabC = document.getElementById('tabCurrent');
        var tabN = document.getElementById('tabNew');
    ...
    if (imgInput) {
        imgInput.addEventListener('change', function(e) {
            ...
            reader.onload = function(ev) {
                ...
                // Also update snapshot thumb
                var snap = document.getElementById('snapshotThumb');
                if (snap && snap.tagName === 'IMG') {
                    snap.src = ev.target.result;
                } else if (snap) {
                    var img = document.createElement('img');
                    img.src = ev.target.result;
                    img.className = 'snapshot-thumb';
                    img.id = 'snapshotThumb';
                    img.alt = 'Preview';
                    snap.replaceWith(img);
                }
            };
