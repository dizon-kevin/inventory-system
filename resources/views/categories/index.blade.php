...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── CATEGORY GRID ── */
        .cat-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 12px; margin-bottom: 1.4rem; }
        ...
        .cat-card:hover { box-shadow: 0 5px 18px rgba(0,0,0,.07); transform: translateY(-2px); border-color: rgba(0,212,170,.2); }

        .cat-card-top { display: flex; align-items: flex-start; justify-content: space-between; gap: .5rem; }
        ...
        .cat-icon-0 { background: rgba(0,212,170,.1);  color: #00a878; }
        .cat-icon-1 { background: rgba(77,166,255,.1);  color: #2563eb; }
        .cat-icon-2 { background: rgba(124,58,237,.1);  color: #7c3aed; }
        .cat-icon-3 { background: rgba(251,191,36,.1);  color: #d97706; }
        .cat-icon-4 { background: rgba(239,68,68,.1);   color: #dc2626; }
        .cat-icon-5 { background: rgba(34,197,94,.1);   color: #16a34a; }

        .cat-id { font-family: 'Space Mono',monospace; font-size: .62rem; color: var(--tm); background: var(--light); border-radius: 5px; padding: 2px 6px; align-self: flex-start; }

        .cat-name { font-size: .9rem; font-weight: 700; color: var(--tp); line-height: 1.3; }

        .cat-count { display: flex; align-items: center; gap: 5px; font-size: .75rem; color: var(--tm); }
        .cat-count svg { color: var(--teal); }

        .cat-actions { display: flex; gap: .4rem; padding-top: .6rem; border-top: 1px solid var(--border); }

        .cat-btn { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; border-radius: 6px; font-size: .72rem; font-weight: 500; text-decoration: none; cursor: pointer; border: 1px solid transparent; font-family: 'Sora',sans-serif; transition: background .14s, color .14s; background: none; flex: 1; justify-content: center; }
        .cat-btn-view { color: #2563eb; border-color: rgba(37,99,235,.18); } .cat-btn-view:hover { background: rgba(37,99,235,.07); }
        .cat-btn-edit { color: #00a878; border-color: rgba(0,168,120,.18); } .cat-btn-edit:hover { background: rgba(0,168,120,.07); }
        .cat-btn-del  { color: #dc2626; border-color: rgba(220,38,38,.18); } .cat-btn-del:hover  { background: rgba(220,38,38,.07); }

        /* ── TABLE CARD (fallback view) ── */
        .table-card { background: var(--card); border: 1px solid var(--border); border-radius: 14px; overflow: hidden; animation: cardIn .5s .2s cubic-bezier(.22,1,.36,1) both; }
        ...
        .toggle-btn { width: 30px; height: 30px; border-radius: 6px; border: 1px solid var(--border); background: transparent; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--tm); transition: background .14s, color .14s; }
        .toggle-btn.active, .toggle-btn:hover { background: var(--dark); color: #d8f0e8; border-color: transparent; }
        ...
        .td-id { font-family: 'Space Mono',monospace; font-size: .72rem; color: var(--tm); }
        .td-primary { font-weight: 600; color: var(--tp); }
        .td-count { display: inline-flex; align-items: center; gap: 5px; font-size: .8rem; color: var(--ts); }
        .td-count svg { color: var(--teal); }

        .actions { display: flex; align-items: center; gap: .4rem; }
        .act-btn { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; border-radius: 6px; font-size: .73rem; font-weight: 500; text-decoration: none; cursor: pointer; border: 1px solid transparent; font-family: 'Sora',sans-serif; transition: background .14s; background: none; }
        .act-view { color: #2563eb; border-color: rgba(37,99,235,.18); } .act-view:hover { background: rgba(37,99,235,.07); }
        ...
        /* Empty state */
        .empty-wrap { padding: 3.5rem 1.4rem; text-align: center; }
        ...
        .empty-sub   { font-size: .8rem; color: var(--tm); margin-bottom: 1.2rem; }
        .btn-empty-add { display: inline-flex; align-items: center; gap: 7px; padding: .65rem 1.2rem; background: var(--dark); color: #d8f0e8; font-family: 'Sora',sans-serif; font-size: .84rem; font-weight: 600; border: none; border-radius: 9px; cursor: pointer; text-decoration: none; transition: background .2s, transform .15s; }
        .btn-empty-add:hover { background: #122a20; transform: translateY(-1px); }
        ...
        .modal-cancel:hover { background: var(--light); }
...
<div class="modal-backdrop" id="deleteModal">
    <div class="modal-box">
        ...
        <p class="modal-body" id="deleteModalMsg">Deleting this category may affect products assigned to it. This action cannot be undone.</p>
...
<div class="shell">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sb-nav">
            ...
            <a href="{{ route('admin.categories.index') }}" class="ni active">
    ...
    <div class="main">
        ...
        <header class="topbar">
            <div class="topbar-left">
                ...
                <p class="topbar-title">Categories</p>
            </div>
        ...
        <main class="content">
            ...
            <div class="page-header">
                <div>
                    ...
                    <p class="page-subtitle">Organise products into groups for easier browsing</p>
                </div>
                <div class="view-toggle" id="viewToggle">
                    <button class="toggle-btn active" id="btnGrid" onclick="setView('grid')" title="Grid view">
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
                    </button>
                    <button class="toggle-btn" id="btnList" onclick="setView('list')" title="List view">
                        ...
                        @foreach($categories as $i => $category)
                            <div class="cat-card" style="animation-delay:{{ $i * 0.04 }}s">
                                <div class="cat-card-top">
                                    <div class="cat-icon cat-icon-{{ $i % 6 }}">
                                ...
                                <div class="cat-actions">
                                    <a href="{{ route('admin.categories.show', $category) }}" class="cat-btn cat-btn-view">
                                    ...
                                    <form id="delForm-{{ $category->id }}" method="POST" action="{{ route('admin.categories.destroy', $category) }}" style="display:none">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                    <button
                                        class="cat-btn cat-btn-del"
                                        onclick="confirmDelete({{ $category->id }}, '{{ addslashes($category->name) }}', {{ $category->products->count() }})"
                                    >
                                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg>
                                        Del
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="table-card">
                        <div class="table-header">
                            <h3 class="table-title">
                                ...
                                All Categories
                                <span class="table-count">{{ $categories->total() }} total</span>
                            </h3>
                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div style="display:flex;align-items:center;gap:9px">
                                                    <div class="cat-icon cat-icon-{{ $i % 6 }}" style="width:30px;height:30px;border-radius:7px">
                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>
                                                    </div>
                                                    <span class="td-primary">{{ $category->name }}</span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="td-count">
                                                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
                                                    {{ $category->products->count() }} {{ Str::plural('product', $category->products->count()) }}
                                                </div>
                                            </td>
                                            <td>
                                                <div class="actions">
                                                    <a href="{{ route('admin.categories.show', $category) }}" class="act-btn act-view">
                                                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                                                        View
                                                    </a>
                                                    <a href="{{ route('admin.categories.edit', $category) }}" class="act-btn act-edit">
                                                    ...
                                                    <button class="act-btn act-del" onclick="confirmDelete({{ $category->id }}, '{{ addslashes($category->name) }}', {{ $category->products->count() }})">
                                                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>
                                                        Delete
                                                    </button>
                                                </div>
                                            </td>
            ...
            @else
                ...
                <div class="table-card">
                    <div class="empty-wrap">
                        ...
                        <p class="empty-sub">Start by creating your first product category.</p>
                        <a href="{{ route('admin.categories.create') }}" class="btn-empty-add">
...
<script>
    ...
    function setView(view) {
        ...
        localStorage.setItem('catView', view);
    }

    // Restore preferred view
    var savedView = localStorage.getItem('catView');
    ...
    function confirmDelete(id, name, productCount) {
        ...
        if (productCount > 0) {
            msg += ' This category has ' + productCount + ' product' + (productCount !== 1 ? 's' : '') + ' assigned to it.';
        }
    ...
    document.getElementById('deleteConfirmBtn').addEventListener('click', function() {
        if (pendingDeleteId) {
            document.getElementById('delForm-' + pendingDeleteId).submit();
        }
    });
